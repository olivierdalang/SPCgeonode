...
start "spcgeonode CeleryCam Docker Log" cmd "/C docker logs spcgeonode_celerycam_1 -f && echo '---TERMINATED---' && timeout 30"
start "spcgeonode RabbitMQ Docker Log" cmd "/C docker logs spcgeonode_rabbitmq_1 -f && echo '---TERMINATED---' && timeout 30"
start "spcgeonode LetsEncrypt Docker Log" cmd "/C docker logs spcgeonode_letsencrypt_1 -f && echo '---TERMINATED---' && timeout 30"
