...
if [ ! -f /spcgeonode-geodatadir/security/spcgeonode_masterpwd_needs_reset_flag ]; then
    ...
    (printf "digest1:" && /usr/lib/jvm/java-1.8-openjdk/jre/bin/java -classpath /geoserver-2.12.1/webapps/geoserver/WEB-INF/lib/jasypt-1.8.jar org.jasypt.intf.cli.JasyptStringDigestCLI digest.sh algorithm=SHA-256 saltSizeBytes=16 iterations=100000 input="$NEW_PASSWORD" verbose=0) | tr -d '\n' > /spcgeonode-geodatadir/security/masterpw.digest
    ...
    /usr/lib/jvm/java-1.8-openjdk/jre/bin/java -classpath /geoserver-2.12.1/webapps/geoserver/WEB-INF/lib/jasypt-1.8.jar org.jasypt.intf.cli.JasyptPBEStringEncryptionCLI input="$NEW_PASSWORD" password='5lp6`U#!rxUuH%nILQqnr3XQPnJd5X$lUFT)cXUU' verbose=0 | tr -d '\n' > /spcgeonode-geodatadir/security/masterpw/default/passwd
