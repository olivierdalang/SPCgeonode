...
if [ ! -f /spcgeonode-geodatadir/security/spcgeonode_masterpwd_needs_reset_flag ]; then
    ...
    OLD_PASSWORD="nRVvOrgcE5pIw7Dgn7T0cPzmCyYNHQsu"
    ...
    # echo "Updating the config:password:key password key"
    # # TODO : we should probably regenerated a config:passowrd:key secret !
    # keytool -keypasswd -alias "config:password:key" -keypass "$OLD_PASSWORD" -new "$NEW_PASSWORD" -keystore /spcgeonode-geodatadir/security/geoserver.jceks -storetype JCEKS -storepass "$OLD_PASSWORD"

    echo "Recreating a random config:password:key secret key"
    keytool -delete -alias "config:password:key" -keypass "$OLD_PASSWORD" -keystore /spcgeonode-geodatadir/security/geoserver.jceks -storetype JCEKS -storepass "$NEW_PASSWORD"
    keytool -genseckey -alias "config:password:key" -keypass "$NEW_PASSWORD" -keystore /spcgeonode-geodatadir/security/geoserver.jceks -storetype JCEKS -storepass "$NEW_PASSWORD"
